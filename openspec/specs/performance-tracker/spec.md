...
Per-strategy performance tracking for live trading. Logs every signal, order, fill, and rejection to disk, computes rolling performance metrics (win rate, PnL, Sharpe, drawdown), and detects when live performance diverges significantly from the deployment backtest baseline.
