...
Long-running service that orchestrates strategy loading, WebSocket bar stream subscriptions, and signal evaluation. Bridges the strategy store (what to watch) with the bar streaming layer (live market data) and feeds completed bars to the signal evaluator. Part of the realtime trading automation engine.
