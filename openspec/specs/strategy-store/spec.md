...
File-based CRUD store for live trading strategy definitions. Each strategy is persisted as a JSON file under `data/strategies/`. Supports hot reload via `fs.watch` so strategy parameter changes take effect without restarting the server.
