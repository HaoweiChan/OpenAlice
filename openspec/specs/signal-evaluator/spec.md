...
Per-strategy rolling window indicator engine and DSL expression evaluator for live trading. Maintains a rolling buffer of OHLCV bars per symbol+timeframe, recomputes technical indicators on each new bar, and evaluates entry/exit DSL expressions to generate trade signals. Reuses the backtester's indicator and DSL evaluation functions.
